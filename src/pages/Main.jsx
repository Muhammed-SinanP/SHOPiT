...
const Main = ({category}) => {
  ...
  return (
    ...
    <div className='m-4 sm:mx-8 md:mx-10 grid grid-cols-12 gap-4'>
     ...
     (list.length > 0 ?
     (list.map((item)=>(
      <SmallCard key={item.id} category={category} productId={item.id} image={item.image} title={item.title} price={item.price} rating={item.rating.rate}/>)
     ))
      
       : 
       "loading..") : "Please Login First"}
    </div>
    </div>
  )
}
